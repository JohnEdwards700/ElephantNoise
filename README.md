...
```bash
cd "/home/jtedwards/Ubuntu_Code/Projects/Elephant_Noise/ElephantNoise/Project/Noise Cleanup"
...
If you want to share the Streamlit app publicly without deploying it to a cloud host, you can tunnel your local app through ngrok.

### 1. Install and authenticate ngrok

Create an ngrok account, install the `ngrok` CLI, then add your auth token:
...
By default, Streamlit serves the app on:

```text
http://localhost:8501
```

### 3. Expose the app through ngrok
...
ngrok will print a public forwarding URL that looks like:

```text
https://abcd-1234.ngrok-free.app
```

Anyone with that URL can reach your local Streamlit app while both the Streamlit process and ngrok process are running.

### 4. Share the public URL

Once the tunnel is live, open the HTTPS ngrok URL in a browser or send it to collaborators.

### Stable URLs and custom domains

- Free ngrok usage typically gives you an ngrok-managed public domain.
- If you want a stable reserved domain or your own custom domain, configure that in ngrok first, then start the tunnel using that reserved domain.
- Custom domains usually require a paid ngrok plan and a DNS record pointing your domain to ngrok.

### Troubleshooting

- If the public page does not load, make sure Streamlit is still running locally on port `8501`.
- If ngrok starts but the app is blank, refresh after the Streamlit app finishes starting.
- If port `8501` is already in use, start Streamlit on another port and tunnel that port instead.
...
An ngrok URL makes your local app reachable from the public internet. Do not expose sensitive files, credentials, or internal-only data through the app unless you add access controls.
